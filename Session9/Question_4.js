const daysInMonths = {
  ...
  class Interval{
    ...
    unwindIntervals(intervalsForDate, strategy) {
      if (strategy === 1) {
          return this.strategy1(intervalsForDate);
      } else {
          return this.strategy2(intervalsForDate);
      }
    }
      ...
      for(let i=0, j=1; i < boundaryCount - 1, j < boundaryCount; i++, j++) {
        ...
        if(j < boundaryCount - 1){
          ...
          if(date === 1){
            ...
            let newDate = daysInMonths[previousMonth];
  ...
  const sinplifierForDates = new Interval();
  const intervalsForDate = [["1-jan", "2-may"], ["2-feb", "30-jun"], ["3-mar", "8-jul"]];
  console.log("Strategy 1: ", sinplifierForDates.unwindIntervals(intervalsForDate, 1));
  console.log("\n");
  console.log("Strategy 2: ", sinplifierForDates.unwindIntervals(intervalsForDate, 2));